...
namespace PayxApi.Controllers
{
    public class EmployeeController : Controller
    {
        ...
        public async Task<IActionResult> EmployeeAllowance(int employeeId)
        {
            var me = 0;
            if(employeeId == 0)
            {
                me = int.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier));
                var meD = await _employeeService.GetEmployeeAllowance(me);
                return View(meD.Data);
            }
            var emp = await _employeeService.GetEmployeeAllowance(employeeId);
            return View(emp.Data);
        }
    }
}